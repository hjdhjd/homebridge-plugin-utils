 ...
 *
 * Mirrors the host package's `./testing` entry point (`src/testing/index.ts`), the one home for shared test infrastructure - this file plays that role for
 * the eslint plugin, sitting alongside its own entry-point files (`plugin.mjs`, `config.mjs`, `index.mjs`) with co-located test files in `rules/` importing
 * back up via `../test-setup.mjs`.
 */
