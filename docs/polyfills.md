...
```ts
type ErmInstallTarget = Partial<Pick<typeof globalThis, "AsyncDisposableStack" | "DisposableStack" | "SuppressedError">>;
```
...
```ts
function installErmPolyfills(target?): void;
```
...
| ------ | ------ | ------ | ------ |
| `target` | [`ErmInstallTarget`](#erminstalltarget) | `globalThis` | The object to install onto. Defaults to `globalThis`. |
