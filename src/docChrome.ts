 ...
 *
 * docChrome.ts: Pure, isomorphic renderers that project a per-plugin documentation-chrome manifest into the masthead, navigation index, dashboard badges, logo, and
 * project list that every family plugin repeats by hand across its README, its docs, and its webUI.
 */
 ...
 * row, and tagline block. See {@link docChromeRegions}.
 *
 * @category Doc Chrome
 */
export interface DocChromeRegionPlan {
...
/**
 * Answer which chrome regions a documentation entry's own file carries. A `"doc"` entry receives both unless it opts out - `masthead: false` drops the masthead,
 * `footer: false` drops the documentation footer index - while an entry of any other kind receives neither, since a README anchor and an external destination name no
 * file of the plugin's own to stamp.
 *
 * The readers of this question have to agree on the answer: the `prepare-chrome` subcommand, which builds its per-file edit plan from it, and a plugin's own
 * documentation-identity test, which checks its committed files against the manifest they were generated from. Answering it once here is what keeps that test from
 * re-deriving the stamper's decision by hand and drifting from it.
 *
 * @param entry - The documentation-index entry to plan.
 *
 * @returns The regions the entry's file carries.
 *
 * @category Doc Chrome
 */
export function docChromeRegions(entry: DocEntry): DocChromeRegionPlan {

  // The opt-outs live on the `"doc"` arm alone, so every other kind answers the same empty plan rather than reading fields it does not carry.
  if(entry.kind !== "doc") {

    return { documentation: false, masthead: false };
  }

  return { documentation: entry.footer !== false, masthead: entry.masthead !== false };
}
...
/**
 * The opening marker of the auto-generated logo region (webUI only). See {@link renderLogo}.
 *
 * @category Doc Chrome
 */
export const LOGO_BEGIN = "<!-- LOGO:BEGIN - Auto-generated by homebridge-plugin-utils. Do not edit this region by hand. -->";

/**
 * The closing marker of the auto-generated logo region. See {@link LOGO_BEGIN}.
 *
 * @category Doc Chrome
 */
export const LOGO_END = "<!-- LOGO:END -->";
...
/**
 * Render the plugin's artwork as the single `<img>` element the webUI page carries, read from the same `masthead.logo` the README's masthead is built from, so one
 * declaration drives both surfaces. The element carries the `chrome-logo` class and nothing else: its sizing, its centering, and the markup wrapped around it stay the
 * page's own, the same way the documentation index and the project list leave the markup surrounding them alone.
 *
 * @param manifest - The documentation-chrome manifest.
 *
 * @returns The rendered image element, a single line.
 *
 * @category Doc Chrome
 */
export function renderLogo(manifest: DocChromeManifest): string {

  const { logo } = manifest.masthead;

  // Both attribute values are author-supplied and land inside double quotes, so they go through the attribute escaper every other HTML surface here uses rather than a
  // second one of their own.
  return "<img class=\"chrome-logo\" src=\"" + escapeHtmlAttr(logo.src) + "\" alt=\"" + escapeHtmlAttr(logo.alt) + "\" />";
}
