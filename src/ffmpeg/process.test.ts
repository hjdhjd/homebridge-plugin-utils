...
describe("FfmpegProcess - construction and readiness", () => {
  ...
  test("args are captured immutably at construction - caller-side mutations do not leak", async () => {
    ...
    // Assert the stored vector positively as well: a view answering with an empty array would satisfy the negative log assertion below while telling a caller nothing
    // true about what the process was spawned with. Frozenness is asserted beside the content because the content assertion alone cannot see the difference: a getter
