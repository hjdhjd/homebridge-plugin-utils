 ...
 *
 * What is left here is this package's policy alone: which major closes the gap, which modules the sweep skips, what its detectors look for, what its checklist
 * enumerates. The mechanism those feed - reading the engines floor, deciding the regime, walking the shipped source, firing the sunset - is the shared machinery in
 * `testing/runtime-floor.ts`, which the whole family drives with its own policy.
 */
import { assertRuntimeFloorCompat, composeSunsetCleanup, parseRuntimeFloor, planRuntimeFloorCheck, readEnginesNode,
  sweepSourceFiles } from "./testing/runtime-floor.ts";
import { describe, test } from "node:test";
import type { ErmInstallTarget } from "./polyfills.ts";
import type { SweptFile } from "./testing/runtime-floor.ts";
...
const SUNSET_CLEANUP = composeSunsetCleanup({

  artifacts: SUNSET_ARTIFACTS,
  epilogue: "; delete the shim import from src/ffmpeg/rtp.ts (the call sites already read against the platform global); and delete this file.",
  prologue: "The Node runtime floor has reached the release that ships the explicit-resource-management globals, so the in-package shims and the polyfill that " +
    "installs them are now redundant. Complete the sunset: delete "
});
...
// construction site exists today and an inline sweep body would therefore never execute. It stays here rather than in the shared machinery because pairing a
// construction with a shim import is this package's own remedy; another repository in the family guards the same gap with an entirely different rule.
...
describe("HBPU runtime floor - regime helper", () => {
  ...
  test("parses the compat floor and selects the compat regime", () => {
    ...
    const result = parseRuntimeFloor({ enginesNode: ">=22.20", sunsetMajor: NODE_ERM_GLOBAL_MAJOR });
  ...
  test("parses a >=24 floor and selects the sunset regime", () => {
    ...
    const result = parseRuntimeFloor({ enginesNode: ">=24", sunsetMajor: NODE_ERM_GLOBAL_MAJOR });
  ...
  test("parses a ^24 floor and selects the sunset regime", () => {
    ...
    assert.equal(parseRuntimeFloor({ enginesNode: "^24", sunsetMajor: NODE_ERM_GLOBAL_MAJOR }).regime, "sunset");
  });

  test("throws on an unparseable engines value", () => {

    assert.throws(() => parseRuntimeFloor({ enginesNode: "not-a-version", sunsetMajor: NODE_ERM_GLOBAL_MAJOR }), /Unable to parse/);
  });

  test("the sunset regime produces the enumerated cleanup plan", () => {

    const plan = planRuntimeFloorCheck({ enginesNode: ">=24", sunsetMajor: NODE_ERM_GLOBAL_MAJOR, sunsetMessage: SUNSET_CLEANUP });
  ...
  test("the compat regime selects the source sweep plan", () => {
    ...
    assert.equal(planRuntimeFloorCheck({ enginesNode: ">=22.20", sunsetMajor: NODE_ERM_GLOBAL_MAJOR, sunsetMessage: SUNSET_CLEANUP }).kind, "sweep");
  });
});
