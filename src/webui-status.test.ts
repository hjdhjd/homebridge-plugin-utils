...
/* Compile-time shape exercises. These never run - the function is never called, and its leading underscore marks it, with its bindings, as a compile-time exercise
 * the typecheck reads - so they add nothing to the runtime totals; TypeScript still type-checks the body during `npm run typecheck`, so a shape regression in the
 * protocol fails the build here rather than silently at a consuming plugin. The negative cases use `@ts-expect-error`, which fails the build if the error it expects
 * ever stops occurring.
 */
const _protocolShapeExercises = (): void => {

  // Every kind of the event union constructs at its declared shape.
  const _hello: StatusEvent = { generation: 1, kind: "hello" };
  const _connecting: StatusEvent = { kind: "connecting", serialNumber: "abc", session: 1 };
  const _snapshot: StatusEvent = { encrypted: true, kind: "snapshot", online: true, rows: [], serialNumber: "abc", session: 2 };
  const _row: StatusEvent = { kind: "row", row: { id: "door", value: "Open" }, serialNumber: "abc", session: 3 };
  const _onlineAvailability: StatusEvent = { encrypted: true, kind: "availability", online: true, serialNumber: "abc", session: 4 };
  const _offlineAvailability: StatusEvent = { encrypted: false, kind: "availability", online: false, serialNumber: "abc", session: 5 };
  const _errored: StatusEvent = { kind: "error", reason: "auth-invalid", serialNumber: "abc", session: 6 };
  ...
  const _misconfigured: StatusEvent = { kind: "error", reason: "misconfigured", serialNumber: "abc", session: 7 };
  const _notReady: StatusEvent = { kind: "error", reason: "not-ready", serialNumber: "abc", session: 8 };
  const _throttled: StatusEvent = { kind: "error", reason: "throttled", serialNumber: "abc", session: 9 };
  const _unsupported: StatusEvent = { kind: "error", reason: "unsupported", serialNumber: "abc", session: 10 };
  ...
  const _fullRow: StatusRow = { ...template, value: "Open" };
  const _latchRow: StatusRow = { id: "motion", label: "Motion", latch: { seconds: 5, value: "Detected" }, sizer: [ "Detected", "Clear" ], value: "Detected" };

  // The view request carries only the identity field.
  const _viewRequest: StatusViewRequest = { serialNumber: "abc" };
  ...
  const _emptySizer: StatusRowTemplate = { id: "x", label: "X", sizer: [] };
  ...
  const _encryptedOffline: StatusEvent = { encrypted: true, kind: "availability", online: false, serialNumber: "abc", session: 7 };
};
 ...
 */
const _rowFormShapeExercises = (): void => {
  ...
  const _untaggedTemplate = { id: "door", label: "Door", sizer: "Stopped (100%)" } satisfies StatusTextRowTemplate;
  const _taggedTemplate = { id: "door", kind: "text", label: "Door", sizer: "Stopped (100%)" } satisfies StatusRowTemplate;
  const _untaggedRow = { id: "door", label: "Door", sizer: "Stopped (100%)", value: "Open" } satisfies StatusRow;
  const _taggedRow = { id: "door", kind: "text", label: "Door", sizer: "Stopped (100%)", value: "Open" } satisfies StatusRow;
  ...
  const _choicesTemplate = { id: "modes", kind: "choices", label: "Modes" } satisfies StatusRowTemplate;
  const _choicesRow = { choices: [ choice, { label: "Home", selected: false } ], id: "modes", kind: "choices", label: "Modes" } satisfies StatusRow;
  ...
  const _textRowEvent: StatusEvent = { kind: "row", row: textUpdate, serialNumber: "abc", session: 1 };
  const _choicesRowEvent: StatusEvent = { kind: "row", row: choicesUpdate, serialNumber: "abc", session: 2 };
  ...
  const _mixedUpdate = { choices: [{ label: "Away", selected: true }], id: "modes", value: "Away" } satisfies StatusRowUpdate;
};
