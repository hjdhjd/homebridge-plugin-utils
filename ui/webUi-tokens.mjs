 ...
 * font scale, spacing scale, layout, radii, motion, accent colors, surfaces, borders, text, form controls, interactive states, specific colors). Cleanup is
...
// scale or alphabet. The groups themselves fall into two clusters: the dimensional groups come first, followed by the color groups, which run from the
// broadest, foundational concerns down to the narrowest, single-purpose tokens.
const buildTokenCss = () => [
  ...
  // The focus ring (box-shadow glow) shared by every dark-mode focus rule and the search field's light-mode rule. Light matches the container border color
  // (--fo-border-accent) so the ring reads as the same theme slate; dark redeclares a lighter accent tint below, because a translucent color that reads well
  // on the dark surface washes out to near-gray on the light one - the ring is genuinely mode-dependent, like the surface tokens.
